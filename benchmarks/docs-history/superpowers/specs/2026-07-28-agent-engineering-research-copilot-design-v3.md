# Agent Engineering Research Copilot 项目规划（v3）
...
### 11.1.1 教程第 8–14 章映射与验收

Phase 2 必须按下表逐章验收，不能只以“页面能打开”作为基线完成标准：

| 教程章节 | v3 对应模块 | 基线验收 |
| --- | --- | --- |
| 第 8 章：项目目标、架构、目录 | Section 8、`app/`、`frontend/` | 目录、启动说明和主链路图可对应源码 |
| 第 9 章：工程底座 | `context`、`monitor`、`llm`、`prompts` | 配置加载、会话上下文和事件封装有单测 |
| 第 10 章：网络搜索助手 | `Web Research Agent`、Tavily tool | 能完成一次搜索并产出工具开始/结束事件 |
| 第 11 章：数据库查询助手 | `Structured Data Agent`、MySQL tool | 能发现表、查看样例、执行受控只读查询 |
| 第 12 章：RAGFlow 助手 | `Knowledge Base Agent`、RAGFlow adapter | 能发现助手并完成一次知识库问答 |
| 第 13 章：主智能体与文件交付 | Main Agent、File tools、Report tools | 能上传附件、读取内容、生成 Markdown/PDF |
| 第 14 章：后端与前端闭环 | FastAPI、WebSocket、React | 能启动任务、取消任务、查看事件并下载产物 |

基线验收必须保存：启动命令、依赖版本、示例输入、事件记录、输出文件和失败处理结果，并在 `v0.1-tutorial-parity` tag 中固定。
...
数据切换通过 profile 完成，而不是手工替换代码：

```text
--profile tutorial       # 教程药品 MySQL、教程 RAGFlow 文档和教程示例任务
--profile agent-research # Agent 研究目录、官方文档/论文和技术决策任务
```

两个 profile 共用工具契约和应用代码，但拥有独立的 SQL dump、RAGFlow collection 配置、样例任务和评测 fixture。最终简历版本使用 `agent-research`，教程 profile 用于复现学习过程和回归测试。

建议里程碑 tag：

```text
v0.1-tutorial-parity   # Phase 0-2：完整教程基线
v0.2-portfolio-core    # Phase 3-6：领域数据、评测、引用、编排、可观测性
v0.3-reliable-runtime  # Phase 7-8：持久化、恢复、审批和成本治理
v1.0-portfolio         # Phase 9：冻结功能后的公开作品集版本
```
...
持久化分两个版本实现，避免在教程复刻阶段提前引入平台复杂度：

| 版本 | Checkpoint | 任务/事件 | 目标 |
| --- | --- | --- | --- |
| `v0.1-tutorial-parity` | 教程使用的内存 checkpoint | 内存事件 + WebSocket | 证明 DeepAgents 主链路和前端闭环 |
| `v0.2-portfolio-core` | 通过适配器封装的持久化 checkpoint | SQLite 或 PostgreSQL 任务/事件表 | 支持固定评测和任务查询 |
| `v0.3-reliable-runtime` | 最终选定的 checkpoint store | 持久化事件、artifact 和审批 | 支持重启恢复、事件续传和幂等执行 |

最终存储选型必须在进入 Phase 7 前通过 ADR 固化；在此之前，业务代码只能依赖 `CheckpointStore`、`TaskStore` 和 `EventStore` 接口，不能直接依赖某个数据库实现。
...
Phase 2 先按教程接口完成闭环，Phase 7 以后再切换到持久化任务接口：

| 教程基线接口 | v3 最终接口 | 迁移说明 |
| --- | --- | --- |
| `POST /api/task` | `POST /api/tasks` | 请求体增加策略、快照和预算配置 |
| `POST /api/task/{thread_id}/cancel` | `POST /api/tasks/{task_id}/cancel` | 从会话标识切换为任务标识 |
| `POST /api/upload` | `POST /api/uploads` | 返回带 `artifact_id` 的文件元数据 |
| `GET /api/files` | `GET /api/artifacts` | 文件列表扩展为报告和中间产物 |
| `GET /api/download` | `GET /api/artifacts/{artifact_id}/download` | 禁止客户端直接拼接文件路径 |
| `WS /ws/{thread_id}` | `WS /ws/tasks/{task_id}` | 增加 `last_event_id` 断线续传 |

基线接口不需要长期兼容，但必须在 `v0.1-tutorial-parity` 的验收记录中保留请求样例和响应样例，避免最终 API 改名后无法证明完成过教程闭环。
...
| Python 环境 | `uv` | `uv --version`、`uv sync` |
| Node.js | LTS 版本 | `node --version` |
| 前端包管理 | `pnpm` | `pnpm --version`、`pnpm install` |
| 容器运行时 | Docker + Compose v2 | `docker compose version` |
| 模型 | OpenAI-compatible API | 最小 chat/structured-output health check |
| 网络搜索 | Tavily API Key | 单次搜索 smoke test |
| 结构化数据 | MySQL Compose 服务 | `list_sql_tables` smoke test |
| 私有知识库 | 外部 RAGFlow 服务 | 助手列表和问答 smoke test |

RAGFlow 不属于教程仓库的 Compose 启动范围，必须在 README 中给出外部部署方式。没有外部 Key 时，测试和离线评测必须使用 mock provider；不能让缺少 RAGFlow 阻塞 Web、MySQL、文件和基线测试。
...
**`v0.1-tutorial-parity`**：教程第 8–14 章全部通过映射表验收，外部依赖和 mock 模式均可运行，能完成一次 Web + MySQL + RAGFlow + 文件报告闭环。

**`v0.2-portfolio-core`**：完成 AI Agent 领域迁移、固定快照、至少 `seed-10`/`dev-40`、引用验证、至少三种编排策略和基础 trace；所有核心指标都有原始评测记录。

**`v0.3-reliable-runtime`**：完成持久化任务、checkpoint、事件续传、故障恢复、人工审批和预算治理；故障注入结果可复现，且没有绕过硬预算的路径。

`v1.0-portfolio` 只负责文档、演示、发布和冻结范围，不再新增核心架构能力。
