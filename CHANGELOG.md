...
- B2 passed local gates (API/WS closure 1 passed, mock integration 30 passed, desktop happy path 1 passed); B3 passed its gates (full Python 353 passed / 11 opt-in skips, B3 focused 11 passed, Vitest 24 passed, desktop Playwright 3 passed / 1 project skip; the pre-existing Starlette httpx deprecation warning remains). The evidence-only finalization is committed as `ed4552f8f3b82ddd4cf097a9c70322a3f46e3215` (parent `27832bc`) and CI-verified by GitHub Actions push run 30907212389 (head ed4552f, success); it changes no runtime/test behavior.
