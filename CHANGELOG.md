...
- Failure/cancel/rerun closure tests (local, uncommitted): `tests/integration/phase2/test_failure_cancel_rerun.py` and `tests/e2e/phase2/test_failure_cancel_rerun_closure.py` exercise provider failure, user cancel, duplicate cancel, and failure-then-rerun over the real HTTP/WebSocket stack, asserting exactly one terminal event per scenario.
...
- B1 CI gate passed on GitHub Actions push run 30878728964 (head 9839440, remote `codex/phase2a-websocket-e2e` = `98394404`): Python 3.12 steps and the frontend Node 22 + pnpm 10 job (frozen install, Chromium install, Vitest, lint, build, Playwright browser tests) are all green. The earlier local Homebrew Node 22.23.2 / pnpm 11.9.0 compatibility rerun remains as historical evidence only.
- B2 and B3 passed local gates (B2: API/WS closure 1 passed, mock integration 30 passed, desktop happy path 1 passed; B3: full Python 353 passed / 11 opt-in skips, B3 focused 11 passed, Vitest 24 passed, desktop Playwright 3 passed / 1 project skip; the pre-existing Starlette httpx deprecation warning remains). **The B3 test files and the B3/B4 documentation changes are not committed** and have no remote CI coverage; user acceptance and a separate authorization to commit and re-run CI are still required.
