...
class TestSessionWorkspace:
    ...
    def test_resolve_output_sanitizes_traversal(self, tmp_path):
        ws = SessionWorkspace.for_thread(
            thread_id="00000000-0000-4000-8000-000000000001",
            base_upload=str(tmp_path / "updated"),
            base_output=str(tmp_path / "output"),
        )
        path = ws.resolve_output("../outside.txt")
        # Basename sanitized, still inside output dir
        assert str(ws.output_dir.resolve()) in str(path.resolve())
        assert path.name == "outside.txt"

    def test_output_path_is_contained(self, tmp_path):
        ws = SessionWorkspace.for_thread(
            thread_id="00000000-0000-4000-8000-000000000001",
            base_upload=str(tmp_path / "updated"),
            base_output=str(tmp_path / "output"),
