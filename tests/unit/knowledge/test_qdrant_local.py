...
def test_qdrant_local_fuses_dense_and_sparse_ranks(tmp_path: Path) -> None:
    index = QdrantLocalKnowledgeIndex(tmp_path / "index", spec(), FakeEmbedder())
    docs = (
        KnowledgeDocument(
            collection_id="fixture-knowledge",
            ...
            chunks=(
                KnowledgeDocumentChunk(
                    chunk_id="chunk-1", content="alpha vector context"
                ),
            ),
        ),
        KnowledgeDocument(
            ...
            chunks=(
                KnowledgeDocumentChunk(
                    chunk_id="chunk-1", content="exact-token exact-token"
                ),
            ),
        ),
    )
    ...
    result = index.search("exact-token", limit=2)

    assert {chunk.document_id for chunk in result} == {"document-1", "document-2"}
    assert result[0].document_id == "document-2"
        ...
        KnowledgeDocument(
            ...
            chunks=(
                KnowledgeDocumentChunk(
                    ...
                    content="Grocery list and weekend plans for the trip.",
                    section_path="Notes",
                ),
            ),
        ),
