...
## 思考水平实验轮（2026-08-30，质量维度补充后修正）
- [x] 质量补充实验（3 题 × 3 水平全文留档 + planner 查询有效性 2/2 客观测量）：
  三水平均无幻觉、思考版信息量略多（+10-20%）但 limitations JSON 结构崩坏
 （132/101 条单字碎片直通解析）——质量增益不足以支撑 8-40 倍成本，
  维持全角色 disabled 结论成立（已有质量维度证据）
- [x] 防御修复：_clean_limitations 剔除 <2 字符碎片条目（两条解析路径统一 + 敌意测试）
- 原实验记录：
...
## 遗留优化轮 L1-L3（2026-08-31）
- [x] L1 B9 段落裁剪对齐 (19698e6)：编造判定存在时无 claim 段落一并裁剪（strict pruning）
- [x] L2 select 级库配额 (60e5bfc)：uploads 系保底 2 条——S4 硬件链路全通（4核/16GB 完整呈现）
