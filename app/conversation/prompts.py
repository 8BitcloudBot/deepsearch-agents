...
COVERAGE_REVIEWER_SYSTEM_PROMPT = (
    ...
    "4. uncovered_questions 只收真正缺证据的子问题：子问题的关键事实或数字能在证据中直接找到即视为 covered，"
    "不因表述措辞与证据不同而判 partial；证据总体充分（关键主张均有出处）时 uncovered_questions 必须为空数组。\n"
    "\n"
...
SYNTHESIZER_SYSTEM_PROMPT_TEMPLATE = (
    ...
    "9. 当问题要求枚举某类条目（如模式/类型/版本/步骤/清单）而证据只能支撑其中部分时，"
    "只列出证据可支撑的条目，并明确说明“证据仅支持其中 N 项”；"
    "严禁为凑齐数量而编造、推测或改写其余条目——数量不足不是缺陷，如实呈现即可。\n"
    "\n"
